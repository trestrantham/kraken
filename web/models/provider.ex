defmodule Kraken.Provider do
  ...
  def status_for_user(query, %User{} = user) do
    ...
    |> select([p, c], %{id: p.id, name: p.name, message: p.message, state: fragment("CASE WHEN ? IS NULL THEN ? WHEN extract(epoch from now()) <= ? THEN 'connected' ELSE 'disconnected' END AS state", c.expires_at, p.state, c.expires_at)})
  end
